...
| NFR-4 | Security and Privacy | 🟨 | backend/app/core/security.py; backend/app/api/v1/dependencies/auth.py; backend/app/core/logging.py; backend/app/main.py; backend/app/connectors/github/client.py; backend/app/api/v1/routes/admin_routes.py; infra/caddy/Caddyfile; backend/app/db/repositories/query_logs_repo.py; backend/app/db/repositories/feedback_repo.py; backend/app/workers/tasks/purge_tasks.py (Step 79 evaluation: AES-256 at-rest, log redaction, least-privilege token enforcement, source/threshold audit logging, and role/permission change audit logging are implemented. Remaining gaps: TLS 1.3 transit enforcement evidence is not implemented in deploy config (`infra/caddy/Caddyfile` currently empty), and 90-day configurable purge for query logs/feedback (NFR-4.5) is not implemented in data repositories/tasks.) | backend/tests/unit/test_api/test_auth.py; backend/tests/integration/test_auth_flow.py; backend/tests/unit/test_core/test_secrets_encryption.py; backend/tests/unit/test_core/test_logging_hygiene.py; backend/tests/unit/test_connectors/test_github_client.py; backend/tests/unit/test_api/test_admin_routes.py; backend/tests/integration/test_audit_logging.py (evidence: Step 78 scoped gate -> 6 unit + 3 integration passed, commit 6ff024a. Missing: `backend/tests/integration/test_data_retention.py` coverage for purge query logs >90d, purge feedback with log, and configurable retention.) | Step 79 |
