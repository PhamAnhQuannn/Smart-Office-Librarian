...
export default function LoginPage(): JSX.Element {
  ...
  // Show error forwarded from Google OAuth callback
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const urlError = params.get("error");
    if (urlError) {
      const messages: Record<string, string> = {
        oauth_denied: "You cancelled Google sign-in.",
        oauth_state_mismatch: "Security check failed. Please try again.",
        account_inactive: "Your account is inactive. Contact support.",
      };
      setError(messages[urlError] ?? "Google sign-in failed. Please try again.");
    }
  }, []);
